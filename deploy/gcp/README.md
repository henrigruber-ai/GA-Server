...
```bash
curl -fsSL \
  https://raw.githubusercontent.com/henrigruber-ai/GA-Server/v0.3.0/deploy/gcp/bootstrap.sh \
...
sudo \
  ...
  GA_DEPLOYMENT_SOURCE_REF=v0.3.0 \
  GA_SERVER_IMAGE_TAG=0.3.0 \
...
```bash
docker pull ghcr.io/henrigruber-ai/ga-server:0.3.0
```
...
- `sha-<commit>`: auf einen konkreten Commit rückführbar und für kontrollierte
  Rollbacks geeignet.
- `0.3.0`: Release-Version; für Produktion bevorzugt und als unveränderlicher
